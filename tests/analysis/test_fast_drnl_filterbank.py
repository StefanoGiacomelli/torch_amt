"""
Fast DRNL Filterbank - Test Suite

Optimized Fast DRNL implementation (~100x faster than original) using FFT-based convolution.

Contents:
1. test_fast_drnl_linear_path: Tests and visualizes linear path (gain → gammatone → lowpass)
2. test_fast_drnl_nonlinear_path: Tests and visualizes nonlinear path (gammatone → compression → lowpass)
3. test_fast_drnl_input_output: Compares input/output response at different SPL levels

Structure:
- Dual-resonance nonlinear (DRNL) filterbank implementation with FFT convolution
- Stage-by-stage signal processing visualization
- Filter frequency response analysis
- Level-dependent compression behavior

Figures generated:
- fast_drnl_linear_path.png: Linear path analysis (9-panel grid)
- fast_drnl_nonlinear_path.png: Nonlinear path analysis (11-panel grid)
- fast_drnl_input_output.png: Input/output comparison (7-panel grid)
"""
...
def test_fast_drnl_linear_path():
    """Test and visualize the linear path of Fast DRNL filterbank."""
    ...
    print("="*80)
    print("Fast DRNL LINEAR PATH TEST")
    print("="*80)
    print(f"\nTest signal: {test_freq} Hz tone @ {test_level} dB SPL")
    print(f"Duration: {dur*1000:.1f} ms")
    print(f"Sampling rate: {fs} Hz")
    
    # Create Fast DRNL filterbank
    drnl = FastDRNLFilterbank((250, 8000), fs=fs, n_channels=50, dtype=torch.float64)
    ...
    plt.suptitle('Fast DRNL Filterbank: Linear Path Analysis', fontsize=14, fontweight='bold')
    plt.tight_layout()
    
    output_file = TEST_FIGURES_DIR / 'fast_drnl_linear_path.png'
...
def test_fast_drnl_nonlinear_path():
    """Test and visualize the nonlinear path of Fast DRNL filterbank with all stages."""
    ...
    print("="*80)
    print("Fast DRNL NONLINEAR PATH TEST")
    print("="*80)
    print(f"\nTest signal: {test_freq} Hz tone @ {test_level} dB SPL")
    print(f"Duration: {dur*1000:.1f} ms")
    print(f"Sampling rate: {fs} Hz")
    ...
    plt.suptitle('Fast DRNL Filterbank: Nonlinear Path Analysis', fontsize=14, fontweight='bold')
    plt.tight_layout()
    
    output_file = TEST_FIGURES_DIR / 'fast_drnl_nonlinear_path.png'
    plt.savefig(output_file, dpi=600, format='png', bbox_inches='tight')
    print(f"\n✓ Saved: {output_file}")
...
def test_fast_drnl_input_output():
    ...
    print("="*80)
    print("Fast DRNL INPUT/OUTPUT COMPARISON TEST")
    print("="*80)
    print(f"\nTest signal: {test_freq} Hz tone @ {test_level} dB SPL")
    print(f"Duration: {dur*1000:.1f} ms")
    print(f"Sampling rate: {fs} Hz")
    ...
    print(f"\nProcessing through full Fast DRNL filterbank...")
    with torch.no_grad():
    ...
    plt.suptitle('Fast DRNL Filterbank: Input/Output Comparison', fontsize=14, fontweight='bold')
    plt.tight_layout()
    
    output_file = TEST_FIGURES_DIR / 'fast_drnl_input_output.png'
    plt.savefig(output_file, dpi=600, format='png', bbox_inches='tight')
    print(f"\n✓ Saved: {output_file}")
...
if __name__ == '__main__':
    test_fast_drnl_linear_path()
    print("\n")
    test_fast_drnl_nonlinear_path()
    print("\n")
    test_fast_drnl_input_output()
